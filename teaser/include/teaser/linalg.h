 ...
 */
Eigen::Matrix<float, 3, 3> hatmap(const Eigen::Matrix<float, 3, 1>& u) {
 ...
 * @tparam NumT numerical type for Eigen matrices (float, float, etc.)
 * @param v1 [in] first vector
 * @param v2 [in] second vector
...
Eigen::Matrix<NumT, Eigen::Dynamic, 1> vectorKron(const Eigen::Matrix<NumT, N, 1>& v1,
  ...
  Eigen::Matrix<float, Eigen::Dynamic, 1> output(v1.rows() * v2.rows(), 1);
#pragma omp parallel for collapse(2) shared(v1, v2, output) default(none)
...
void getNearestPSD(const Eigen::Matrix<NumT, Eigen::Dynamic, Eigen::Dynamic>& A,
  ...
  // symmetrize A into B
  Eigen::MatrixXf B = (A + A.transpose()) / 2;

  // eigendecomposition of B
  Eigen::SelfAdjointEigenSolver<Eigen::MatrixXf> eig_B(B);
  Eigen::VectorXf De = eig_B.eigenvalues();
  Eigen::MatrixXf De_positive = (De.array() < 0).select(0, De).asDiagonal();
  Eigen::MatrixXf Ve = eig_B.eigenvectors();
